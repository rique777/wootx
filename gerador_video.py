...
def carregar_chaves_eleven():
    with open("eleven_keys.txt", "r") as f:
...
# Função para gerar narração com Eleven Labs em português brasileiro usando várias chaves
def gerar_audio(historia, id_voz):
    print("Gerando áudio da narração em português brasileiro...")
    keys = carregar_chaves_eleven()
    for eleven_api_key in keys:
        ...
        if response.status_code == 200:
            ...
            print(f"Erro ao gerar narração com a chave atual, tentando próxima chave. Status: {response.status_code}")
    print("Falha ao gerar narração após tentar todas as chaves.")
    return None
...
for usuario in usuarios:
    ...
    if audio:
        ...
        print("Erro na geração de áudio. Vídeo não criado para o usuário:", usuario['usuario'])
